<div class="offset-10-laptop section_asside_block">
  <div class="section">

    <?php echo $session->presentationPage("En cas d'accident", "bg7"); ?>

    <div class="sesction-body accident-block">
      <h2 class="margin-bottom-none">EN CAS D’ACCIDENT DE TRAVAIL QUE FAISONS-NOUS ? </h2>

      <p class="margin-top-none">
        Nous assistons rapidement la victime d’un accident de travail une de nos priorités. Leur réintégration nous tient vivement à cœur. Nous tenons également à contribuer à la réduction des couts liés à la
        CNESST.
      </p>
      <p>
        Nous prendrons des mesures pour vous assister dans l’élaboration de vos dossiers et pour arriver à une réduction des coûts au minimum. De plus, nous vous accompagnerons dans la gestion de vos dossiers CNESST depuis la date de l’événement jusqu’à la fermeture complète du dossier.
      </p>
      <p>
        Nos services dans ce cas sont entre autres :
      </p>

      <ul>
        <li>	Un coaching et un suivi réguliers auprès de la personne-ressource d’<strong>Alpha Omega Solutions Travail Inc.</strong>;</li>
        <li>	Une analyse minutieuse des dossiers actifs suivie des recommandations,</li>
        <li>	Une intervention rapide auprès de la CNESST;</li>
        <li>	 Des expertises médicales;</li>
        <li>	Une demande de partage des coûts qui en découlent;</li>
        <li>
          Une aide possible à la préparation pour les auditions au Tribunal Administratif du Travail (TAT). Pour plus de détails sur la procédure à suivre en cas d’accident de travail, consultez le site de la CNESST:
          <a href="https://www.csst.qc.ca/travailleurs/accident-du-travail-ou-maladie.../accident-travail.aspx" target="_blank">www.csst.qc.ca/travailleurs/accident-du-travail-ou-maladie.../accident-travail.aspx</a>
        </li>
      </ul>

      <p>
        Si vous avez des questions, veuillez <a href="<?php echo _ROOT_PATH; ?>contact-us/contact-us/">communiquer</a> sans délai avec notre responsable de la gestion SST.
      </p>
    </div>
  </div>
  <aside class="aside">
    <?php include(_APP_PATH."/outils/php/aside.php"); ?>
  </aside>
</div>
