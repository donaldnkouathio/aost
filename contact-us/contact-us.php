<div class="offset-10-laptop contacteznous-block">

  <div class="left">
    <h2 class="margin-bottom-none">Siège social</h2>
    <div class="footer-flex">
      <span class="material-icons margin-right-5 icon">location_on</span>
      <a target="_blank" href='https://goo.gl/maps/rfbuu5FmwwKd8qr39'>
        2232 Rue Noel, H4M1R9 Montréal
      </a>
    </div>

    <iframe class="aost-map" src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d2796.556710652994!2d-73.68411138463804!3d45.498870739150696!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x4cc91823f9bcd0f3%3A0x62315c3f751bd3a7!2s2232%20Rue%20No%C3%ABl%2C%20Saint-Laurent%2C%20QC%20H4M%201R9%2C%20Canada!5e0!3m2!1sfr!2s!4v1639257524932!5m2!1sfr!2s" style="border:0;" allowfullscreen="" loading="lazy"></iframe>


    <h2 class="margin-bottom-none">Bureau principal</h2>
    <div class="footer-flex">
      <span class="material-icons margin-right-5 icon">location_on</span>
      <a target="_blank" href='https://goo.gl/maps/rFA6r9KnfB9g1CHL7'>
        880 BOULEVARD DECARIE, H4L1M3, Montréal, Saint-Laurent, Québec, Canada
      </a>
    </div>

    <iframe class="aost-map" src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d2795.8871379375964!2d-73.68222358463767!3d45.51235053825053!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x4cc9183ffeb1547b%3A0x83402b7ca27aa26b!2s880%20Bd%20D%C3%A9carie%2C%20Saint-Laurent%2C%20QC%20H4L%203M1%2C%20Canada!5e0!3m2!1sfr!2sfr!4v1642459196512!5m2!1sfr!2sfr" style="border:0;" allowfullscreen="" loading="lazy"></iframe>


    <h2 class="margin-bottom-none">Contacts</h2>
    <div class="footer-flex">
      <span class="notranslate material-icons margin-right-5 icon">phone</span>
      <a href='tel:+1 514-612-0043'>+1 514-612-0043</a> - Bureau
    </div>
    <div class="footer-flex">
      <span class="notranslate material-icons margin-right-5 icon">phone</span>
      <a href='tel:+1 514-559-7850'>+1 514-559-7850</a> - Direction
    </div>
    <div class="footer-flex">
      <span class="notranslate material-icons margin-right-5 icon">phone</span>
      <a href='tel:+1 514-621-7850'>+1 514-621-7850</a> - Direction
    </div>


    <h2 class="margin-bottom-none">Courriels</h2>
    <div class="footer-flex">
      <span class="notranslate material-icons margin-right-5 icon">email</span>
      <a class="notranslate" href='mailto:info@alphaomegasolutionstravail.ca'> Info@alphaomegasolutionstravail.ca (Informations)</a>
    </div>
    <div class="footer-flex">
      <span class="notranslate material-icons margin-right-5 icon">email</span>
      <a class="notranslate" href='mailto:rh@alphaomegasolutionstravail.ca'> RH@alphaomegasolutionstravail.ca (Demande d'emploi)</a>
    </div>
    <div class="footer-flex">
      <span class="notranslate material-icons margin-right-5 icon">email</span>
      <a class="notranslate" href='mailto:paie@alphaomegasolutionstravail.ca'> Paie@alphaomegasolutionstravail.ca (Paie)</a>
    </div>
    <div class="footer-flex">
      <span class="notranslate material-icons margin-right-5 icon">email</span>
      <a class="notranslate" href='mailto:dr@alphaomegasolutionstravail.ca'> DR@alphaomegasolutionstravail.ca (Directeur)</a>
    </div>
    <div class="footer-flex">
      <span class="notranslate material-icons margin-right-5 icon">email</span>
      <a class="notranslate" href='mailto:partenaires@alphaomegasolutionstravail.ca'> Partenaires@alphaomegasolutionstravail.ca (Partenaires)</a>
    </div>


    <h2 class="margin-bottom-none">Réseaux sociaux</h2>
    <div class="footer-flex">
      <a href="https://www.facebook.com/ALPHA-OMEGA-Solutions-Travail-AOST-120693572651635/" rel="noopener" target="_blank">
        Facebook
      </a>
    </div>

  </div>

  <div class="right">
    <h2 class="margin-bottom-none">Vous avez une question ?</h2>

    <form class="" id="contact_form">
      <div class="input-block">
        <label for="compagnie">Compagnie <span class="form-required"> </span> :</label>
        <input type="text" name="compagny" required value="">
      </div>
      <div class="input-block">
        <label for="ville">Ville où vous êtes <span class="form-required"> </span> :</label>
        <input type="text" name="city" required value="">
      </div>
      <div class="input-block">
        <label for="industrie">Type d'industrie <span class="form-required"> </span> :</label>
        <input type="text" name="compagny_type" required value="">
      </div>
      <div class="input-block">
        <label for="personneRessource">Personne-ressource <span class="form-required"> </span> :</label>
        <input type="text" name="person" required value="">
      </div>
      <div class="input-block">
        <label for="telephone">Téléphone <span class="form-required"> </span> :</label>
        <input type="number" name="phone" required value="">
      </div>
      <div class="input-block">
        <label for="telecopieur">Télécopieur <span class="form-required"> </span> :</label>
        <input type="number" name="fax_phone" required value="">
      </div>
      <div class="input-block">
        <label for="courriel">Courriel <span class="form-required"> </span> :</label>
        <input type="email" name="email" required value="">
      </div>
      <div class="input-block text-area-block">
        <label for="besoin">Décrivez votre besoin <span class="form-required"> </span> :</label>
        <textarea name="need" required></textarea>
      </div>

      <div class="button-block">
        <button type="submit" id="btn_submit_contact" class="btn btn-primary" name="button">Envoyer</button>
      </div>
    </form>
  </div>

</div>

<div class="alert_modal_shadow alert_modal_shadow_request">
  <div class="alert_modal_block">
    <div class="alert_modal_body">
      <i class="notranslate  material-icons">check_circle</i>
      <span>Envoyé avec success</span>
    </div>

    <div class="alert_modal_footer">
      <button type="button" class="btn alert_modal_close" name="button">Fermer</button>
    </div>
  </div>
</div>

<script>
  $(document).ready(function(){
    // hide modal
   function hideModal(modal, modal_btn_close){
     $(".alert_modal_block").click(function(ev){
       ev.stopPropagation();
     });
     modal_btn_close.click(function(){
       modal.fadeOut();
     });
    }

    hideModal($(".alert_modal_shadow_request"), $(".alert_modal_close, .alert_modal_shadow"));

    var btn_submit_contact = $('#btn_submit_contact');

    //form for contact us
    $('#contact_form').submit(function(ev){
      ev.preventDefault();
      var formData = new FormData($('#contact_form')[0]);

      $.ajax({
        url: "../outils/php/traitement/request/add-request.php",
        type: "POST",
        data: formData,
        beforeSend: function(){
          btn_submit_contact.prop("disabled", true);
          btn_submit_contact.html('<span class="btn-loading"><span class="loader"></span></span>');
        },
        success: function(ret){
          btn_submit_contact.prop("disabled", false);
          btn_submit_contact.html("Envoyer");
          $(".alert_modal_shadow_request").fadeIn();

          $('#contact_form')[0].reset();
        },
        cache: false,
        contentType: false,
        processData: false
      });
    });

  });
</script>
